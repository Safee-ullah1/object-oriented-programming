...
def store_user(category, user_name, password):
    ...
    if category == "staff":
        ...
        user_atts.append(0)
    save_users(user_names, user_passwords, user_atts)
    save_staff(user_names, user_passwords)
...
def save_users(names, passwords, attendances):
    with open("residents.txt", 'w') as file:
        for name, password, attendance in zip(names, passwords, attendances):
            print(name, password, attendance, sep=',', file=file)


def save_staff(names, passwords):
    with open("staff.txt", 'w') as file:
        for name, password in zip(names, passwords):
            print(name, password, sep=',', file=file)
            

def save_notices(notices):
    with open("notices.txt", 'w') as file:
        for notice in notices:
            print(notice, file=file)
...
def read_users():
    ...
    with open("residents.txt", 'r') as file:
        for line in   file.readlines():
            if len(line.split(',')) ==  3:
                name, password, attendance = line.split(',')
                user_names.append(name)
                user_passwords.append(password)
                user_atts.append(attendance.split('\n')[0])
            
            
def read_staff():
    global staff_names
    global staff_passwords
    staff_names = []
    staff_passwords = []
    with open("staff.txt", 'r') as file:
        for line in   file.readlines():
            if len(line.split(',')) ==  2:
                ...
                staff_passwords.append(password.split('\n')[0])
